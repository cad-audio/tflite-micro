...
TfLiteStatus AverageReferenceEvalInt8(TfLiteContext* context, TfLiteNode* node) {
...
TfLiteStatus MaxReferenceEvalInt8(TfLiteContext* context, TfLiteNode* node) {
  TFLITE_DCHECK(node->builtin_data != nullptr);
  auto* params = reinterpret_cast<TfLitePoolParams*>(node->builtin_data);
