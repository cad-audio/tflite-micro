...
TfLiteStatus AverageReferenceEvalInt16(TfLiteContext* context, TfLiteNode* node) {
...
TfLiteStatus MaxReferenceEvalInt16(TfLiteContext* context, TfLiteNode* node) {
  TFLITE_DCHECK(node->builtin_data != nullptr);
  auto* params = reinterpret_cast<TfLitePoolParams*>(node->builtin_data);
...
TFLMRegistration Register_AVERAGE_POOL_2D_INT16REF() {
  return tflite::micro::RegisterOp(XtensaPoolingInit, PoolingPrepare,
                                   AverageReferenceEvalInt16);
}

TFLMRegistration Register_MAX_POOL_2D_INT16REF() {
  return tflite::micro::RegisterOp(XtensaPoolingInit, PoolingPrepare,
                                   MaxReferenceEvalInt16);
}
