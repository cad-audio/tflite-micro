...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    ...
    case kTfLiteInt8: {
      ...
      int input_dimension = input_shape.DimensionsCount();
      if((output_shape.Dims(0)==1) && (input_dimension ==3))
      {
        if((output_shape.Dims(1) == input_shape.Dims(1)) && (output_shape.Dims(2) == input_shape.Dims(2)))
        {
          int batch = op_params.begin[2];
          memcpy(output->data.raw, input->data.raw + batch*output_bytes, output_bytes);
        }
      }
      else
      {
...
TfLiteRegistration Register_SLICE() {
  return tflite::micro::RegisterOp(NULL, Prepare, Eval);
}
