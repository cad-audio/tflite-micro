...
#endif // #if defined(INCLUDE_FLOAT_OPT) && (defined(HIFI3) || defined(HIFI4) || defined(HIFI5))
...
TfLiteStatus TransposeConvEvalHifiFloat32(TfLiteContext* context, TfLiteNode* node,
    ...
    if(bias != NULL){
      ...
      const RuntimeShape& output_shape =
          tflite::micro::GetTensorShape(output);
      float* scratch_buffer = static_cast<float*>(
          context->GetScratchBuffer(context, data.scratch_buffer_index));
      const int stride_width = data.params.stride_width;
      const int stride_height = data.params.stride_height;
      const int pad_width = data.params.padding_values.width;
      ...
      for (int b = 0; b < batches; b++) {
        xa_nn_transpose_conv_f32(
            ...
            const_cast<float*>(
                  &input_data[b * input_height * input_width * input_depth]),
            const_cast<float*>(filter_data), const_cast<float*>(bias_data),
            stride_width, stride_height, pad_width, pad_height, input_depth,
            output_depth, input_height, input_width, filter_height,
            filter_width, output_height, output_width, num_elements / batches,
            scratch_buffer);
      ...
      xa_nn_vec_activation_min_max_f32_f32(output_data, output_data, output_activation_min,
...
#endif  // defined(HIFI3) || defined(HIFI3) || defined(HIFI4) || defined(HIFI5)
